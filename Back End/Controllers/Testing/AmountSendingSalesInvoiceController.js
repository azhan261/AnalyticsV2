...
var { AmountSendingSalesInvoice } = require ("../../Models/Testing/AmountSendingSalesInvoice")
 ...
 router.get('/', (req, res) => {
     AmountSendingSalesInvoice.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving AmountSendingSalesInvoice :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    AmountSendingSalesInvoice.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving AmountSendingSalesInvoice :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    AmountSendingSalesInvoice.findById(id, (err, addAccount) => {
      res.json(addAccount);
    });
  });
 ...
 router.post('/create', (req, res) => {
     var emp = new AmountSendingSalesInvoice({
 ...
 /*router.post('/:id', (req, res) => {
    ...
    AmountSendingSalesInvoice.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in AmountSendingSalesInvoice Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  AmountSendingSalesInvoice.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving AmountSendingSalesInvoice :' + JSON.stringify(err, undifines, 2)); }
  });
