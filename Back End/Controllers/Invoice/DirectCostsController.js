...
var { DirectCostsSchema } = require ("../../Models/Invoice/DirectCostsSchema")
 ...
 router.get('/', (req, res) => {
     DirectCostsSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving DirectCostsSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    DirectCostsSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving DirectCostsSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    DirectCostsSchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 /*router.post('/:id', (req, res) => {
    ...
    DirectCostsSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in DirectCostsSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  DirectCostsSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving DirectCostsSchema :' + JSON.stringify(err, undifines, 2)); }
  });
