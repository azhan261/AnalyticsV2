...
var { CurrentAssetSchema } = require ("../../Models/Invoice/CurrentAssetSchema")
 ...
 router.get('/', (req, res) => {
     CurrentAssetSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving CurrentAssetSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    CurrentAssetSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving CurrentAssetSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    CurrentAssetSchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 router.post('/create', (req, res) => {
     var emp = new CurrentAssetSchema({
      from:req.body.from,
 ...
 /*router.post('/:id', (req, res) => {
    ...
    CurrentAssetSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in CurrentAssetSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  CurrentAssetSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving CurrentAssetSchema :' + JSON.stringify(err, undifines, 2)); }
  });
