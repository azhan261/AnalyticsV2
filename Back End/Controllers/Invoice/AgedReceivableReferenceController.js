...
var { AgedReceivableReferenceSchema } = require("../../Models/Invoice/AgedReceivableReferenceSchema")
 ...
 router.get('/', (req, res) => {
     AgedReceivableReferenceSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving AgedReceivableReferenceSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    AgedReceivableReferenceSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving AgedReceivableReferenceSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    AgedReceivableReferenceSchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 router.post('/create', (req, res) => {
     var emp = new AgedReceivableReferenceSchema({
      to:req.body.to,
      startDate: req.body.to,
 ...
 /*router.post('/:id', (req, res) => {
    ...
    AgedReceivableReferenceSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in AgedReceivableReferenceSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  AgedReceivableReferenceSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving AgedReceivableReferenceSchema :' + JSON.stringify(err, undifines, 2)); }
  });
