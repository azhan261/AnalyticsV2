...
var { NonCurrentAssetSchema } = require ("../../Models/Invoice/NonCurrentAssetSchema")
 ...
 router.get('/', (req, res) => {
     NonCurrentAssetSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving NonCurrentAssetSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    NonCurrentAssetSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving NonCurrentAssetSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    NonCurrentAssetSchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 router.post('/create', (req, res) => {
     var emp = new NonCurrentAssetSchema({
      from:req.body.from,
 ...
 /*router.post('/:id', (req, res) => {
    ...
    NonCurrentAssetSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in NonCurrentAssetSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  NonCurrentAssetSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving NonCurrentAssetSchema :' + JSON.stringify(err, undifines, 2)); }
  });
