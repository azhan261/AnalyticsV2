...
var { NonCurrentLiabilitySchema } = require ("../../Models/Invoice/NonCurrentLiabilitySchema")
 ...
 router.get('/', (req, res) => {
     NonCurrentLiabilitySchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving NonCurrentLiabilitySchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    NonCurrentLiabilitySchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving NonCurrentLiabilitySchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    NonCurrentLiabilitySchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 router.post('/create', (req, res) => {
     var emp = new NonCurrentLiabilitySchema({
      to:req.body.to,
 ...
 /*router.post('/:id', (req, res) => {
    ...
    NonCurrentLiabilitySchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in NonCurrentLiabilitySchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  NonCurrentLiabilitySchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving NonCurrentLiabilitySchema :' + JSON.stringify(err, undifines, 2)); }
  });
