...
var { FixedAssetSchema } = require ("../../Models/Invoice/FixedAssetSchema")
 ...
 router.get('/', (req, res) => {
     FixedAssetSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving FixedAssetSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    FixedAssetSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving FixedAssetSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    FixedAssetSchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 /*router.post('/:id', (req, res) => {
    ...
    FixedAssetSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in FixedAssetSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  FixedAssetSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving FixedAssetSchema :' + JSON.stringify(err, undifines, 2)); }
  });
