...
var { PurchaseInvoice } = require ("../../Models/Invoice/PurchaseInvoice")
 ...
 router.get('/', (req, res) => {
     PurchaseInvoice.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving PurchaseInvoice :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    PurchaseInvoice.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving PurchaseInvoice :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    PurchaseInvoice.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 /*router.post('/:id', (req, res) => {
    ...
    PurchaseInvoice.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in PurchaseInvoice Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  PurchaseInvoice.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving PurchaseInvoice :' + JSON.stringify(err, undifines, 2)); }
  });
