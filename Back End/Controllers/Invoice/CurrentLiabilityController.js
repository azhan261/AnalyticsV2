...
var { CurrentLiabilitySchema } = require ("../../Models/Invoice/CurrentLiabilitySchema")
 ...
 router.get('/', (req, res) => {
     CurrentLiabilitySchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving CurrentLiabilitySchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    CurrentLiabilitySchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving CurrentLiabilitySchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    CurrentLiabilitySchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 router.post('/create', (req, res) => {
     var emp = new CurrentLiabilitySchema({
      to:req.body.to,
 ...
 /*router.post('/:id', (req, res) => {
    ...
    CurrentLiabilitySchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in CurrentLiabilitySchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  CurrentLiabilitySchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving CurrentLiabilitySchema :' + JSON.stringify(err, undifines, 2)); }
  });
