...
var { AgedPayableReferenceSchema } = require("../../Models/Invoice/AgedPayableReferenceSchema")
 ...
 router.get('/', (req, res) => {
     AgedPayableReferenceSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving AgedPayableReferenceSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    AgedPayableReferenceSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving AgedPayableReferenceSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    AgedPayableReferenceSchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 router.post('/create', (req, res) => {
     var emp = new AgedPayableReferenceSchema({
      to:req.body.to,
 ...
 /*router.post('/:id', (req, res) => {
    ...
    AgedPayableReferenceSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in AgedPayableReferenceSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  AgedPayableReferenceSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving AgedPayableReferenceSchema :' + JSON.stringify(err, undifines, 2)); }
  });
