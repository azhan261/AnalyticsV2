...
var { EquitySchema } = require ("../../Models/Invoice/EquitySchema")
 ...
 router.get('/', (req, res) => {
     EquitySchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving EquitySchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    EquitySchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving EquitySchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    EquitySchema.findById(id, (err, invoice) => {
      res.json(invoice);
    });
  });
 ...
 /*router.post('/:id', (req, res) => {
    ...
    EquitySchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in EquitySchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  EquitySchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving EquitySchema :' + JSON.stringify(err, undifines, 2)); }
  });
