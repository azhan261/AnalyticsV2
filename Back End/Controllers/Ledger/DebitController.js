...
var { DebitLedgerSchema } = require ("../../Models/Ledger/DebitLedgerSchema")
 ...
 router.get('/', (req, res) => {
     DebitLedgerSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving DebitLedgerSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    DebitLedgerSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving DebitLedgerSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    DebitLedgerSchema.findById(id, (err, totalDebit) => {
      res.json(totalDebit);
    });
  });
 ...
 /*router.post('/:id', (req, res) => {
    ...
    DebitLedgerSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in DebitLedgerSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 router.post("/:id", (req, res) => {
    ...
    DebitLedgerSchema.findById(id, (err, totalDebit) => {
      if (!totalDebit) {
        res.status(404).send("totalDebit not found");
      } else {
        totalDebit.amountTotalDebit = req.body.amountTotalDebit;
        totalDebit.transactionType = req.body.transactionType;
        totalDebit
          .save()
          .then((totalDebit) => {
            res.json(totalDebit);
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  DebitLedgerSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving DebitLedgerSchema :' + JSON.stringify(err, undifines, 2)); }
  });
