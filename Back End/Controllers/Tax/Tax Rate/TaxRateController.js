...
var { TaxRateSchema } = require ("../../../Models/Tax/Tax Rate/TaxRateSchema")
 ...
 router.get('/', (req, res) => {
     TaxRateSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving TaxRateSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.get('/approved', (req, res) => {
    TaxRateSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving TaxRateSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.get("/:id", (req, res) => {
    ...
    TaxRateSchema.findById(id, (err, taxRate) => {
      res.json(taxRate);
    });
  });
 ...
 router.post('/create', (req, res) => {
     var emp = new TaxRateSchema({
      taxRateName:req.body.taxRateName,
      taxRateAccount: req.body.taxRateAccount,
      taxRateAmount: req.body.taxRateAmount,
     });
 ...
 /*router.post('/:id', (req, res) => {
    ...
    TaxRateSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in TaxRateSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 router.post("/:id", (req, res) => {
    ...
    TaxRateSchema.findById(id, (err, taxRate) => {
      if (!taxRate) {
        res.status(404).send("taxRate not found");
      } else {
        taxRate.taxRateName = req.body.taxRateName;
        taxRate.taxRateAccount = req.body.taxRateAccount;
        taxRate.taxRateAmount = req.body.taxRateAmount;
        taxRate
          .save()
          .then((taxRate) => {
            res.json(taxRate);
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  TaxRateSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving TaxRateSchema :' + JSON.stringify(err, undifines, 2)); }
  });
